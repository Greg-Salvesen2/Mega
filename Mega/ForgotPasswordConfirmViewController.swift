...
class ForgotPasswordConfirmViewController: SuperViewController {
    ...
    override func viewDidLoad() {
        ...
        let sendPasswordGestureRecognizer = UITapGestureRecognizer(target: self, action: #selector(ForgotPasswordConfirmViewController.sendPasswordPressed))
        ...
        //Code and New Password Text Fields
        let codeView: UIView = self.addUIView(self.view, x: (width / 2) - (textFieldWidth / 2), y: 280, width: textFieldWidth, height: 60, backgroundRed: 255.0, backgroundGreen: 255.0, backgroundBlue: 255.0, transparency: 255.0, rounded: 30.0)
        codeTextField = self.addUITextField(codeView, x: 20, y: 0, width: codeView.frame.size.width - 20, height: codeView.frame.size.height, placeholderText: "EMAIL")
        ...
        self.addUILabel(sendPasswordView, x: 0, y: 0, width: sendPasswordView.frame.size.width, height: sendPasswordView.frame.size.height, labelText: "RESET PASSWORD", red: 0, green: 0, blue: 0, centered: true, fontSize: kDefaultFontSize)
